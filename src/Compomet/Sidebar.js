...
export default function MiniDrawer() {
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      {/* <CssBaseline /> */}
      <AppBar position="fixed" open={open} style={{backgroundColor:"#575f72"}}>
        <Toolbar>
          <IconButton
            color="inherit"
      ...
      <Drawer variant="permanent" open={open}>
        <DrawerHeader>
          <IconButton onClick={handleDrawerClose}>
            <h5 className='me-3'>Billing OverView</h5>
        ...
        <Divider />
        <List>
          {['Dashboard', 'Patient'].map((text, index) => (
        ...
        <List>
          {[ 'Appointment', 'Billing'].map((text, index) => (
            <ListItem key={text} disablePadding sx={{ display: 'block' }}>
              <ListItemButton
                sx={{
                  minHeight: 48,
              ...
              >
                ...
                >
                  {index % 2 === 0 ? <BookOnlineIcon /> : <InboxIcon /> }
                </ListItemIcon>
